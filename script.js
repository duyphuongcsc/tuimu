...
function createGift() {
  ...
  // Vị trí ngẫu nhiên theo chiều ngang
  const randomX = Math.random() * (window.innerWidth - 50); // Trừ đi kích thước gift
  gift.style.left = randomX + 'px';
  gift.style.top = '-50px';

  // Tạo hình ảnh treo
  const hangingImages = [
    'image/51ce8fee8cfe31a068ef.jpg', // Thay thế bằng các đường dẫn khác
    // Thêm nhiều hình ảnh hơn nếu cần
  ];

  // Chọn ngẫu nhiên một hình ảnh
  const randomImage = hangingImages[Math.floor(Math.random() * hangingImages.length)];

  const hangingImage = document.createElement('img');
  hangingImage.src = randomImage; // Đường dẫn đến hình ảnh treo ngẫu nhiên
  hangingImage.style.width = '80px'; // Kích thước hình ảnh
  hangingImage.style.position = 'absolute';
  hangingImage.style.top = '-20px'; // Đặt vị trí của hình ảnh treo
  hangingImage.style.left = '50%';
  hangingImage.style.transform = 'translateX(-10%)'; // Căn giữa

  gift.appendChild(hangingImage); // Thêm hình ảnh vào hộp quà
  ...
  gift.addEventListener('click', () => {
    ...
    // Thiết lập vị trí ngẫu nhiên cho popup
    const randomX = Math.random() * (window.innerWidth - 150); // 150 là chiều rộng tối đa của popup
    const randomY = Math.random() * (window.innerHeight - 50); // 50 là chiều cao tối đa của popup
...
setInterval(createGift, 1000); // 1 giây một lần
...
function createFirework(x, y) {
  const colors =  ['#ff0', '#ff4', '#4ff', '#f4f', '#4f4', '#f00', '#00f'];
  const particles = 30;
